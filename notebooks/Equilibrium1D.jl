...
function plot(sol, sys)
    ...
    scalarplot!(
        vis, grid, clear = false, calc_p(sol, sys) / ufac"GPa", color = :magenta, label = "p/GPa",
...
scalarplot(
    grid, ysum(sys_pp, sol_pp), xlimits = (0, 5nm), show = true,
...
function capsplot(sys, molarities)
    ...
    for imol in 1:length(molarities)
        ...
        LiquidElectrolytes.set_molarity!(sys.physics.data, molarities[imol])
        t = @elapsed volts, caps = dlcapsweep_equi(sys, vmax = 1V, nsteps = 100)
...
function create_pbsystem()
    ...
    return pbsys = PBSystem(grid; celldata, bcondition)
end
...
function pbcapsplot(sys, molarities)
    ...
    for imol in 1:length(molarities)
        ...
        t = @elapsed result = dlcapsweep(pbsys; inival = unknowns(pbsys, inival = 0), iϕ = 1, voltages)
        (; voltages, dlcaps) = result
        cdl0 = dlcap0(sys.physics.data)
        @info "elapsed=$(t)"
        scalarplot!(
            vis, voltages, dlcaps / (μF / cm^2),
            color = c, clear = false, label = "$(molarities[imol])M", markershape = :none
        )
        scalarplot!(
            vis, [0], [cdl0] / (μF / cm^2),
