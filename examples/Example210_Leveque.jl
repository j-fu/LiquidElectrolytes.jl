...
function main(
    ...
    function convdiff(f, u, edge, data)
        ...
        bp = bp * Diff
        bm = bm * Diff
        return f[1] = bp * u[1] - bm * u[2]
    end

    function outflow(f, u, node, data)
        return if node.region == bout
...
function runtests()
    ...
    return @test main(Cylindrical2D; microscale = true, verbose = false) ≈ 3.500919519297249e8
end
