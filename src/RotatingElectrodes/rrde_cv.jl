...
function CVContext(
    ...
    function bstorage(f, u, node, data)
        return if node.region == b_disk
            for ispec in (num_species + 1):(num_species + num_disk_species)
                f[ispec] = u[ispec]
            end
        end
    end
...
function run_cv(
    ...
    sampling_times = [ i * period / 2 for i in 0:(2 * num_periods)]
    ...
    function pre(sol, time)
        return cvcontext.set_voltage(phi_cv(time))
    end
    ...
    function delta(sys, solution, oldsolution, time, tstep)
        ...
        return di = abs(cvcontext.I_disk(I_disk_old) - cvcontext.I_disk(I_disk)) / mA
    end
    ...
    return (frequency = frequency, vdisk = vdisk, iring = iring, idisk = idisk)
end
...
function add_cvresult!(cvresults, cvresult)
    ...
    return push!(cvresults.idisks, cvresult.idisk)
end


plot_cvresults(cvresults; Plotter = nothing, file = "cv.png", clear = true) = pyplot_cv(
    cvresults.frequencies, cvresults.vdisks, cvresults.idisks, cvresults.irings;
