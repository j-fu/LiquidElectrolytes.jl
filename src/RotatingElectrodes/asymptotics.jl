...
function idisk_levich(self, omega, nu, D)
    ...
    Sc = nu / D
    return FaradayConstant * self.r_1 * self.r_1 * π * (omega * nu)^(1.0 / 2) * 0.6205 * Sc^(-2.0 / 3.0)
end

function iring_levich(self, omega, nu, D)
    FaradayConstant = ph"AvogadroConstant" * ph"ElementaryCharge"
    return (
        FaradayConstant * π * (1.0 / (1.61))
            * D^(2.0 / 3.0)
            * nu^(-1.0 / 6.0)
            * omega^(1.0 / 2.0)
...
function idisk_newman(self, omega, nu, D)
    FaradayConstant = ph"AvogadroConstant" * ph"ElementaryCharge"
    Sc = nu / D
    return FaradayConstant * self.r_1 * self.r_1 * π * (omega * nu)^(1.0 / 2) * 0.62048 * Sc^(-2.0 / 3.0) / (1.0 + 0.298 * Sc^(-1.0 / 3.0) + 0.14514 * Sc^(-2.0 / 3.0))
end
...
function coleff_albery(self)
    function F(theta)
        ...
        return (
            (sqrt(3.0) / (4.0 * π)) * log((1.0 + thetapd)^3 / (1.0 + theta))
